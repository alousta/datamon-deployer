...
if [ "$#" -lt 1 ]
then
  echo "Usage: createSecurityClusterPrereqs.sh oumEnabled where oumEnabled value could either 0 or 1 to disable/enable OUM feature"
  exit 1
fi
...
if supports_scc; then
  ...
  then
    ...
    oc apply -f ibm-cdu-scc-oum.yaml --validate=false
  fi
  isApplied="true"
fi
...
if supports_psp; then
  ...
  if [ "$isApplied" == "false" ]; then
    ...
    then
      ...
      kubectl apply -f ibm-cdu-psp-oum.yaml
    fi
    echo "Creating Role"
