...
{{- end }}
      ...
      runAsUser: {{ .Values.storageSecurity.runAsUser | default 45678 }} 
      runAsGroup:  {{ .Values.storageSecurity.runAsGroup | default 45678 }}
    containerSecurityContext:
      ...
      capabilities:
        drop: [ "ALL" ]
      {{- if eq (toString .Values.oum.enabled | lower) "n"  }}
        add: [ "CHOWN", "SETGID", "SETUID", "DAC_OVERRIDE", "FOWNER", "AUDIT_WRITE", "SYS_CHROOT" ]
      {{- else }}
        add: [ "SETGID", "SETUID", "DAC_OVERRIDE", "AUDIT_WRITE" ]
      {{- end }}
{{- end -}}
